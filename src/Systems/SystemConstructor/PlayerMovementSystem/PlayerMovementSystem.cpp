...
SystemResponse PlayerMovementSystem::update()
{
    ...
    for (size_t i = 0; i < entities.size(); i++)
    {
        try
        {
            ...
            updateVelocityOnInput(keys, velocityComp, playerControllerComp);
...
void PlayerMovementSystem::updateVelocityOnInput(std::map<ecs::input::Key, IRenderManager::KEY_STATE> &keys,
                                                 std::shared_ptr<ecs::components::Velocity> &velocityComp, std::shared_ptr<ecs::components::PlayerController> &playerComp)
{
    std::map<ecs::actions::Action, ecs::input::Key> keyMap = playerComp->getMovementKeys();
    if (keys[keyMap[ecs::actions::LEFT]] == IRenderManager::PRESSED)
        velocityComp->setVelocityX(-180);
    else if (keys[keyMap[ecs::actions::RIGHT]] == IRenderManager::PRESSED)
        velocityComp->setVelocityX(180);
    else
        velocityComp->setVelocityX(0);
    if (keys[keyMap[ecs::actions::UP]] == IRenderManager::PRESSED)
        velocityComp->setVelocityY(-180);
    else if (keys[keyMap[ecs::actions::DOWN]] == IRenderManager::PRESSED)
        velocityComp->setVelocityY(180);
    else
